...
export const POST: RequestHandler = async ({ params, request }) => {
	try {
		...
		const stream = new ReadableStream<Uint8Array>({
			...
			cancel() { controller = undefined; stopConversation(conversationId); }
		});
		const send = (event: string, data: unknown) => {
			if (!controller) return;
			try { controller.enqueue(encoder.encode(sse(event, data))); } catch { /* stream closed */ }
		};
		const close = () => { if (controller) { try { controller.close(); } catch { /* already closed */ } controller = undefined; } };

		void (async () => {
			try {
				send('message.start', { type: 'message.start', messageId: userMessage.id, role: 'user', content: parsed.data.content });
				await runConversationTurn(conversationId, parsed.data.model, parsed.data.content, (event) => send(event.type, event));
				send('done', { type: 'done' });
			} catch (error) {
