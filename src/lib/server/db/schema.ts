...
import { pgTable, text, timestamp, uuid, jsonb, integer, index, primaryKey } from 'drizzle-orm/pg-core';
...
export const conversations = pgTable('conversations', {
	...
	enabledTools: jsonb('enabled_tools').$type<string[]>().notNull().default(sql`'[]'::jsonb`),
	createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
	updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull()
}, (table) => ({ userIdx: index('conversations_user_idx').on(table.userId), projectIdx: index('conversations_project_idx').on(table.projectId), updatedIdx: index('conversations_updated_idx').on(table.updatedAt) }));
...
export const projectFileChunks = pgTable('project_file_chunks', {
	id: uuid('id').defaultRandom().primaryKey(),
	projectId: uuid('project_id').notNull().references(() => projects.id, { onDelete: 'cascade' }),
